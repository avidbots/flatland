...
b2Island::b2Island(
  int32 bodyCapacity, int32 contactCapacity, int32 jointCapacity, b2StackAllocator * allocator,
...
void b2Island::Solve(
  b2Profile * profile, const b2TimeStep & step, const b2Vec2 & gravity, bool allowSleep)
{
  b2Timer timer;
  ...
  if (allowSleep) {
    ...
    for (int32 i = 0; i < m_bodyCount; ++i) {
      ...
      if (
        (b->m_flags & b2Body::e_autoSleepFlag) == 0 ||
